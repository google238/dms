			...
			if cmd == 'devTodo':
				...
				if 'actionID' in content.keys() and 'inst' in content.keys():
					if content['actionID'] == '6':#空调或者空净设备
						if content['inst'].startswith('T'):#空净
						...
						data_model('devcmd').insert(insertitem)
					#	finditem = data_model('action').find_one(conditions={'devid':devID,'inst':{'$regex':'0'}})
					#	if finditem is None:
					#		pass
					#	else:
					#		time0 = finditem['time']
					#		data_model('action').remove(condition={'devid':devID,'time':{'$lte':time0}})
						data_model('action').insert(insertitem)		
						if content['inst'].startswith('0'):
							...
							#	findtemp = data_model('live.area.temp.history').find(conditions={'city':cityname,'area':area,'time':{'$lte':starttime}},limit=1,sort=[('time',-1)])
				...
				if 'socketOut_P' in content.keys() and content['socketOut_P']> 0:
					...
					#按照国电通的白名单转存数据
					findid = data_model('whitelist_gdt').find_one(conditions={'_id':devID})
					if findid is None:
						pass
					else:	
				...
				#处理功率为0
					...
					if findone is None or findone[0]['P']>0: 
						insertdata = {'time':time,'devid':devID,'W':W,'P':P}				
						data_model(tablename).insert(insertdata)
					elif findone[0]['P'] == 0:
				...
				data_model('dev_position').update_set(condition={'_id':devID},values={'city':city,'area':area,'sender':sender,'time':time,'username':username},upsert=True)
				
				appid = content.get('appid','')
				if appid == 'guodiantong':
					data_model('whitelist_gdt').update_set(condition={'_id':devID},values={'city':city,'area':area,'sender':sender,'time':time,'username':username},upsert=True)
